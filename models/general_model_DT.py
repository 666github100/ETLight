...
device = torch.device('cuda:0')###cuda test!
class GeneralDecisionTransformer(NetworkAgent):
    ...
    def build_network(self):
        ...
        network = DecisionTransformer(state_dim=12*self.num_feat,
            ...
            resid_pdrop=0.3,
            attn_pdrop=0.3,)
        network = network.to(device)###cuda test!
        return network
    ...
    def choose_action(self, states):
        ...
        batch_Xs1,actions,rewards,time_steps = batch_Xs1.to(device),actions.to(device),rewards.to(device),time_steps.to(device) ###cuda test!
    ...
    def train_network(self, memory):
        ...
        optimizer = torch.optim.Adam(params=self.model.parameters(), lr=0.000001)
        for epoch in range(epochs):
            for ba in range(int(num_batch)):
                ...
                batch_Xs1,batch_a,batch_r,time_step = batch_Xs1.to(device),batch_a.to(device),batch_r.to(device),time_step.to(device) ###cuda test!
                _, pre_actions, _ = self.model(batch_Xs1, batch_a, batch_r, time_step)
                batch_a = torch.tensor(_action[ba*batch_size:(ba+1)*batch_size]).type(torch.long)###让batch_a符合类型要求
                pre_actions, batch_a = pre_actions.to(device), batch_a.to(device)###cuda test!
