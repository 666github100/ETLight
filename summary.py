...
def get_metrics(duration_list, queue_length_list,traffic_name, total_summary_metrics, num_of_out):
    ...
    validation_queue = queue_length_list[-validation_duration_length:]
    ...
    final_queue = np.round(np.mean(validation_queue), decimals=2)
    ...
    total_summary_metrics["final_queue"].append(final_queue)
...
def summary_detail_RL(memo_rl, total_summary_rl):
    ...
    for traffic_file in os.listdir(records_dir):
        ...
        for round_rl in round_files:
            ...
            for inter_index in range(num_intersection):
                try:
                    ...
                    for sample in samples:
                        queue_length_each_inter_each_round += sum(sample[2]['lane_queue_vehicle_in'])
                    queue_length_each_inter_each_round = queue_length_each_inter_each_round/len(samples)  ####len(samples)=240
                    f.close() 
            ...
            ave_duration = vehicle_duration.mean()  # mean among all the vehicles
            ave_queue_length = np.mean(queue_length_each_round)# the average queue length at each intersection
        ...
        total_summary_rl = get_metrics(duration_each_round_list,queue_length_each_round_list, traffic_file, total_summary_rl, num_of_vehicle_out)
...
def parse_args():
    ...
    parser.add_argument("-memo",   type=str,      default='etlight')
